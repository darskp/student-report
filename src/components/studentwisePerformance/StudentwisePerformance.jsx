          ...
          <Table>
            ...
            <TableBody>
              {data.length === 0 ? (
                ...
                data.map((row, index) => (
                  <TableRow key={row.id} sx={{ backgroundColor: getColor(row.percentage), border: '1px solid #d5cfc7' }}>
                    <TableCell className='table-cell-custom'>
                      <Typography variant="h6">{index + 1}</Typography>
                    </TableCell>
                    <TableCell className='table-cell-custom'>
                      <Typography variant="h6" sx={{ fontWeight: "bold" }}>{row.studentName}</Typography>
                    </TableCell>
                    <TableCell className='table-cell-custom'>
                      <Typography variant="h6">{row.concept}</Typography>
                    </TableCell>
                    <TableCell className='table-cell-custom textalign-start'>
                      {row.subConcept?.map((subConceptItem) => (
                        <Typography key={subConceptItem.id} variant="h6">
                          {subConceptItem.name}
                        </Typography>
                      ))}
                    </TableCell>
                    <TableCell className='table-cell-custom textalign-start'>
                      {row.subConcept?.map((subConceptItem) => (
                        subConceptItem?.rootConcepts?.map((rootConceptItem) => (
                          <Typography key={rootConceptItem.id} variant="h6">
                            {rootConceptItem.name}
                          </Typography>
                        ))
                      ))}
                    </TableCell>
